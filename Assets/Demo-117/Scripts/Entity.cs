...
namespace Demo_117
{
    ...
    public class Entity : ServiceLocator<Entity>
    {
        ...
        public override void Init()
        {
            ...
            //存储服务
            RegisterService<IPrefsService>(new PlayerPrefsService());
            ...
            //场景加载服务
            RegisterService<ISceneService>(new AddressablesSceneService());
            ...
            //TMP字体服务（用于处理文本相关的逻辑，例如字体管理、文本样式等），这里只用来处理TMP_Settings加载的问题
            //如果使用Resources则不需要这个
            RegisterService<ITMPService>(new TMPService());
            ...
            //玩家信息服务（用于处理玩家相关的数据和逻辑）
            RegisterService<IProfileService>(new ProfileService());
            
            //对象池服务（用于管理游戏中的对象池逻辑）
            RegisterService<IObjectPoolService>(new ObjectPoolService());
            
            //粒子服务（用于处理游戏中的粒子效果逻辑）
            RegisterService<IParticleService>(new ParticleService());
